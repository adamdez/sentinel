...
// ── Row builder (mirrors use-leads-by-status.ts) ────────────────────
...
const FSBO_SOURCES = ["facebook", "craigslist", "fb", "fb_craigslist", "fsbo", "zillow_fsbo", "fsbo_com"];

export type FsboSortField = "composite_score" | "updated_at" | "owner_name" | "address";

interface UseFsboLeadsOptions {
  search?: string;
  sortField?: FsboSortField;
  sortDir?: "asc" | "desc";
}

export function useFsboLeads(opts: UseFsboLeadsOptions = {}) {
  ...
  const fetchFromApi = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/leads/by-source?sources=${FSBO_SOURCES.join(",")}`);
      if (!res.ok) {
      ...
      console.error("[useFsboLeads] Error:", err);
  ...
  // Real-time: refetch on DB changes for leads with FSBO sources
  useEffect(() => {
    const channel = supabase
      .channel("fsbo_leads_realtime")
      .on(
        "postgres_changes",
