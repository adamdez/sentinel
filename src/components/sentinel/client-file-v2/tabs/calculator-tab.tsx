...
export function DealCalculatorTab({ arv, initialRepairs = 40000 }: DealCalculatorProps) {
  const [repairs, setRepairs] = useState(initialRepairs);
  const [maoPercentage, setMaoPercentage] = useState(70);

  const mao = (arv * (maoPercentage / 100)) - repairs;
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider block mb-1.5">Target Purchase % (Standard 70%)</label>
      ...
      <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-xl p-6 flex flex-col justify-center relative overflow-hidden">
        ...
        <p className="text-[10px] text-muted-foreground mb-6 font-mono">(ARV × {maoPercentage}%) − Repairs</p>
