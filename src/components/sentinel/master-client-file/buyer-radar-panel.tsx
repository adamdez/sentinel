 ...
 * BuyerRadarPanel
 *
 * Shows a ranked buyer list for a given lead. Opens on demand with no auto-fetch.
 * Operators see the buyer list, action state, flags, and the computed
 * monetizability score once enough active buyers exist.
 */
...
export function BuyerRadarPanel({ leadId }: BuyerRadarPanelProps) {
  ...
  const handleContact = useCallback(
    async (buyerId: string, existing: ExistingDealBuyer | null, dealId: string | null) => {
      if (!dealId) {
        toast.error("No deal found for this lead. Create a deal first.");
        return;
      }

      setActionBuyers((prev) => ({ ...prev, [buyerId]: "loading" }));
      ...
      try {
        ...
        setActionBuyers((prev) => ({ ...prev, [buyerId]: "queued" }));
        toast.success("Buyer queued for outreach");
        refetch();
      } catch (err) {
        setActionBuyers((prev) => {
          const next = { ...prev };
          delete next[buyerId];
          return next;
        });
  ...
  const handleDismiss = useCallback(
    async (buyerId: string, existing: ExistingDealBuyer | null, dealId: string | null) => {
      ...
      try {
        ...
        setActionBuyers((prev) => ({ ...prev, [buyerId]: "passed" }));
    ...
    <div className="rounded-[12px] border border-overlay-6 bg-overlay-2 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          {data && !loading && (
            <span className="text-sm text-muted-foreground/60">{data.activeBuyerCount} active</span>
          )}
        </div>
      ...
      {open && (
        <div className="border-t border-overlay-6 px-3 pb-3 pt-2 space-y-2">
          ...
          {data?.monetizabilityVisible && data.monetizabilityScore !== null && (
            <div className="flex items-center justify-between rounded-[8px] bg-primary/5 border border-primary/10 px-2.5 py-2">
              <span className="text-sm text-primary/70 uppercase tracking-wider font-semibold">
                Computed Monetizability
              </span>
              <div className="flex items-center gap-1.5">
                <span className="text-sm font-mono font-semibold text-primary">
                  {data.monetizabilityScore.toFixed(1)}
                </span>
          ...
          {data && !data.monetizabilityVisible && (
            <div className="text-sm text-muted-foreground/40 italic px-0.5">
              Score available after {10 - data.activeBuyerCount} more active buyer
              {(10 - data.activeBuyerCount) !== 1 ? "s" : ""}
            </div>
          )}
  ...
  return (
    <div
      className={cn(
        ...
        isEliminated ? "border-overlay-4 bg-overlay-2 opacity-40" : "border-overlay-6 bg-overlay-3",
      )}
    >
      <div className="flex items-start gap-2">
        <div className="flex-1 min-w-0 space-y-1">
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {buyer.company_name && <span className="text-sm text-muted-foreground/50">- {buyer.company_name}</span>}
          ...
          {(staleFlag || flags.length > 0 || eliminateReason) && (
            <div className="flex items-center gap-1.5 flex-wrap mt-0.5">
              ...
              {flags.map((flag, index) => (
                <span key={index} className="text-[9.5px] text-muted-foreground/50 bg-overlay-4 rounded px-1.5 py-0.5">
                  {flag}
                </span>
              ))}
              {eliminateReason && <span className="text-[9.5px] text-foreground/50">{eliminateReason}</span>}
            </div>
          )}
        </div>
...
function StatusBadge({ status }: { status: string }) {
  ...
  const config = cfg[status];
  if (!config) return null;

  return <span className={cn("text-xs px-1.5 py-0.5 rounded border font-medium", config.className)}>{config.label}</span>;
}
