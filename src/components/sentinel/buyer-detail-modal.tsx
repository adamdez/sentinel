...
function GlassInput({ value, onChange, placeholder, type = "text", className }: {
  ...
  return (
    <input
      ...
      className={cn(
        "w-full bg-white/[0.03] border border-white/[0.06] rounded-[8px] px-3 py-1.5 text-sm text-foreground placeholder:text-muted-foreground/40 focus:outline-none focus:border-cyan/30 focus:ring-1 focus:ring-cyan/20 transition-all",
        className
      )}
    />
  );
...
function GlassSelect({ value, onChange, options, placeholder }: {
  ...
  return (
    <select
      value={value ?? ""}
      onChange={(e) => onChange(e.target.value)}
      className="w-full bg-white/[0.03] border border-white/[0.06] rounded-[8px] px-3 py-1.5 text-sm text-foreground focus:outline-none focus:border-cyan/30 focus:ring-1 focus:ring-cyan/20 transition-all appearance-none cursor-pointer"
    >
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <button
              onClick={onClose}
              className="absolute right-4 top-4 rounded-[8px] opacity-60 hover:opacity-100 hover:bg-cyan/5 p-1 transition-all z-10"
            >
              <X className="h-4 w-4" />
            </button>
            ...
            <div className="flex items-start gap-3 pr-8">
              <div className="h-10 w-10 rounded-[12px] bg-cyan/8 border border-cyan/18 flex items-center justify-center shrink-0">
                <Building2 className="h-5 w-5 text-cyan/70" />
              </div>
              <div className="flex-1 min-w-0">
                <GlassInput
                  ...
                  className="text-lg font-semibold bg-transparent border-transparent hover:border-white/[0.06] focus:border-cyan/30 px-1 py-0"
                />
                <GlassInput
                  value={form.company_name ?? ""}
                  onChange={(v) => updateField("company_name", v || null)}
                  placeholder="Company (optional)"
                  className="text-sm text-muted-foreground bg-transparent border-transparent hover:border-white/[0.06] focus:border-cyan/30 px-1 py-0 mt-0.5"
                />
              </div>
                              ...
                              <button
                                ...
                                className={cn(
                                  ...
                                  form.preferred_contact_method === m
                                    ? "bg-cyan/12 border-cyan/25 text-cyan"
                ...
                {!collapsedSections.notes && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                    <textarea
                      ...
                      rows={3}
                      className="w-full mt-2 bg-white/[0.03] border border-white/[0.06] rounded-[8px] px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground/40 focus:outline-none focus:border-cyan/30 focus:ring-1 focus:ring-cyan/20 transition-all resize-none"
                    />
                  </motion.div>
                )}
              </AnimatePresence>
            ...
            <div className="flex items-center gap-3 pt-2 border-t border-white/[0.04]">
              {!isCreate && (
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 text-xs text-red-400/80 hover:text-red-400 hover:bg-red-500/10 rounded-[8px] border border-transparent hover:border-red-500/20 transition-all disabled:opacity-40"
                >
              ...
              <button
                onClick={handleSave}
                disabled={saving}
                className="px-4 py-1.5 text-sm font-medium text-cyan bg-cyan/10 hover:bg-cyan/15 rounded-[8px] border border-cyan/25 hover:border-cyan/40 transition-all disabled:opacity-50"
              >
