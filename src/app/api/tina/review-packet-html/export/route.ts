...
export async function POST(req: NextRequest) {
  ...
  if (draft.cpaHandoff.status !== "complete" || draft.cpaHandoff.artifacts.length === 0) {
    return NextResponse.json(
      { error: "CPA handoff packet must be built before HTML export." },
      { status: 409 }
    );
  }
  ...
  try {
    const exportPacket = savedPacket
      ? savedPacket
      : (await persistTinaPacketVersion(sb, user.id, draft, "review_packet_html_export")).packet;
    const exportFile = buildTinaReviewPacketHtmlExport(exportPacket.draft, {
