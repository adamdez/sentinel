...
export async function POST(req: NextRequest) {
  ...
  if (action === "queue") {
    const queuedWorkItem =
      workItem.challengeRun.status === "queued" || workItem.challengeRun.status === "running"
        ? workItem
        : queueTinaAuthorityChallengeRun(workItem);
  ...
  if (!shouldProcessTinaAuthorityBackgroundRun(workItem.challengeRun)) {
    return NextResponse.json({ workItem });
  }

  const runningWorkItem = startTinaAuthorityChallengeRun(workItem);
  ...
  const processedTask = await processTinaAuthorityQueueTask({
    ...
    runChallenge: runTinaAuthorityChallenge,
  });
