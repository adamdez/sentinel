...
import { captureTinaPackageSnapshot } from "@/tina/lib/workflow-state";
...
export async function POST(req: NextRequest) {
  ...
  const updatedDraft = captureTinaPackageSnapshot(draft);

  return NextResponse.json({
    draft: updatedDraft,
    snapshot: updatedDraft.packageSnapshots[0] ?? null,
    capturedBy: user.id,
  });
}
