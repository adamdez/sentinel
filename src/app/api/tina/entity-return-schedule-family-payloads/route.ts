...
export async function GET(req: NextRequest) {
  ...
  if (error) {
    return NextResponse.json(
      { error: "Failed to load Tina entity return schedule-family payload artifacts" },
      { status: 500 }
    );
  }
  ...
  return NextResponse.json({
    entityReturnScheduleFamilyPayloads: buildTinaEntityReturnScheduleFamilyPayloads(draft),
  });
}
