/**
 * GET /api/gmail/inbox?user_id=xxx
 *
 * Fetches the last 10 inbox messages from the user's connected Gmail.
 *
 * Charter v3.0 §4: Service role for DB reads of encrypted tokens.
 */
...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: "Inbox fetch failed", detail: err instanceof Error ? err.message : String(err) },
      { status: 500 }
    );
  }
}
