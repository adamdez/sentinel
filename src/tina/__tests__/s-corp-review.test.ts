...
describe("buildTinaSCorpReviewReport", () => {
  it("builds a real 1120-S review spine from a strong intake packet", () => {
    ...
    const draft = {
      ...
      sourceFacts: [
        ...
        },
        {
          ...
          value: "This paper mentions owner draws.",
          confidence: "medium" as const,
          capturedAt: "2026-04-08T22:04:10.000Z",
        },
        {
          id: "payroll",
    ...
    const report = buildTinaSCorpReviewReport(draft);

    expect(report.status).toBe("needs_review");
    expect(report.sections.some((section) => section.id === "basis_and_capital")).toBe(true);
    expect(report.sections.some((section) => section.id === "shareholder_distributions")).toBe(true);
  });
});
