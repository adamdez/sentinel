...
export const leadStatusEnum = pgEnum("lead_status", [
  "staging", "prospect", "lead", "active", "negotiation", "disposition", "nurture", "dead", "closed",
]);
...
export const leads = pgTable("leads", {
  ...
  dialQueueActive: boolean("dial_queue_active").notNull().default(false),
  dialQueueAddedAt: timestamp("dial_queue_added_at", { withTimezone: true }),
  dialQueueAddedBy: uuid("dial_queue_added_by"),
  introSopActive: boolean("intro_sop_active").notNull().default(true),
  introDayCount: integer("intro_day_count").notNull().default(0),
  introLastCallDate: date("intro_last_call_date"),
  introCompletedAt: timestamp("intro_completed_at", { withTimezone: true }),
  introExitCategory: text("intro_exit_category"),
  introExitReason: text("intro_exit_reason"),
  ...
  skipTraceStatus: text("skip_trace_status").notNull().default("not_started"),
  skipTraceCompletedAt: timestamp("skip_trace_completed_at", { withTimezone: true }),
  skipTraceLastAttemptedAt: timestamp("skip_trace_last_attempted_at", { withTimezone: true }),
  skipTraceLastError: text("skip_trace_last_error"),
  ...
  index("idx_leads_dial_queue").on(table.assignedTo, table.dialQueueActive, table.dialQueueAddedAt),
  index("idx_leads_intro_sop_active").on(table.introSopActive, table.introDayCount, table.introCompletedAt),
  index("idx_leads_intro_exit_category").on(table.introExitCategory),
  index("idx_leads_skip_trace_status").on(table.skipTraceStatus, table.skipTraceCompletedAt),
...
export const tasks = pgTable("tasks", {
  ...
  jeffInteractionId: uuid("jeff_interaction_id"),
  notes: text("notes"),
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => [
  ...
  index("idx_tasks_source").on(table.sourceType, table.sourceKey),
  index("idx_tasks_voice_session").on(table.voiceSessionId),
  index("idx_tasks_jeff_interaction").on(table.jeffInteractionId),
...
export const jeffInteractions = pgTable("jeff_interactions", {
  id: uuid("id").defaultRandom().primaryKey(),
  voiceSessionId: uuid("voice_session_id").notNull(),
  leadId: uuid("lead_id").references(() => leads.id, { onDelete: "set null" }),
  ...
  callbackTimingText: text("callback_timing_text"),
  transferOutcome: text("transfer_outcome"),
  assignedTo: uuid("assigned_to"),
  taskId: uuid("task_id"),
  policyVersion: text("policy_version").notNull(),
  metadata: jsonb("metadata").notNull().default({}),
