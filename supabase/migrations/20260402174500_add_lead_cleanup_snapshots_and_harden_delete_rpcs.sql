CREATE TABLE IF NOT EXISTS "public"."lead_cleanup_snapshots" (
  "id" uuid DEFAULT gen_random_uuid() PRIMARY KEY,
  "cleanup_run_id" uuid NOT NULL UNIQUE,
  "filter" jsonb NOT NULL DEFAULT '{}'::jsonb,
  "summary" jsonb NOT NULL DEFAULT '{}'::jsonb,
  "snapshot" jsonb NOT NULL DEFAULT '{}'::jsonb,
  "status" varchar(32) NOT NULL DEFAULT 'snapshotted',
...
CREATE INDEX IF NOT EXISTS "idx_lead_cleanup_snapshots_status"
  ON "public"."lead_cleanup_snapshots" ("status");

CREATE INDEX IF NOT EXISTS "idx_lead_cleanup_snapshots_executed"
  ON "public"."lead_cleanup_snapshots" ("executed_at");
...
BEGIN
  ...
  IF v_property_id IS NOT NULL THEN
    IF NOT EXISTS (SELECT 1 FROM leads WHERE property_id = v_property_id)
      AND NOT EXISTS (SELECT 1 FROM deals WHERE property_id = v_property_id)
      AND NOT EXISTS (SELECT 1 FROM calls_log WHERE property_id = v_property_id)
      AND NOT EXISTS (SELECT 1 FROM lead_phones WHERE property_id = v_property_id)
      AND NOT EXISTS (SELECT 1 FROM dossiers WHERE property_id = v_property_id)
      AND NOT EXISTS (SELECT 1 FROM dossier_artifacts WHERE property_id = v_property_id)
      AND NOT EXISTS (SELECT 1 FROM research_runs WHERE property_id = v_property_id)
      AND NOT EXISTS (SELECT 1 FROM recorded_documents WHERE property_id = v_property_id)
    THEN
  ...
  RETURN jsonb_build_object(
    'success', true,
    'property_deleted', (v_deleted_properties > 0)
  );
EXCEPTION
  WHEN OTHERS THEN
    BEGIN
...
BEGIN
  ...
  IF COALESCE(array_length(v_property_ids, 1), 0) > 0 THEN
    SELECT array_agg(candidate_property_id)
    INTO v_orphan_property_ids
    FROM unnest(v_property_ids) AS candidate_property_id
    WHERE candidate_property_id IS NOT NULL
      AND NOT EXISTS (SELECT 1 FROM leads WHERE property_id = candidate_property_id)
      AND NOT EXISTS (SELECT 1 FROM deals WHERE property_id = candidate_property_id)
      AND NOT EXISTS (SELECT 1 FROM calls_log WHERE property_id = candidate_property_id)
      AND NOT EXISTS (SELECT 1 FROM lead_phones WHERE property_id = candidate_property_id)
      AND NOT EXISTS (SELECT 1 FROM dossiers WHERE property_id = candidate_property_id)
      AND NOT EXISTS (SELECT 1 FROM dossier_artifacts WHERE property_id = candidate_property_id)
      AND NOT EXISTS (SELECT 1 FROM research_runs WHERE property_id = candidate_property_id)
      AND NOT EXISTS (SELECT 1 FROM recorded_documents WHERE property_id = candidate_property_id);
